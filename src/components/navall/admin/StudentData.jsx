...
const StudentsData = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      setLoading(true);
      try {
        const response = await fetch('https://courseapi-3kus.onrender.com/api/students');
  ...
  const handleDelete = async (email) => {
    if (window.confirm('Are you sure you want to delete this student?')) {
      try {
        const response = await fetch(`https://courseapi-3kus.onrender.com/api/student/${email}`, {
          method: 'DELETE',
        });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://courseapi-3kus.onrender.com/api/student/${selectedStudent.email}`, {
        method: 'PUT',
  ...
  return (
    <div className="">
      ...
      <div className="p-8 bg-white min-h-screen text-black">
        ...
        {/* Stream Selection */}
        <div className="mb-4">
          <label htmlFor="stream" className="text-sm font-medium text-gray-700">Select Stream:</label>
          <select
            id="stream"
            ...
            <table className="min-w-full bg-gray-200 border border-gray-300 rounded-lg overflow-hidden">
              <thead>
                <tr className="bg-gray-300">
                  ...
                  <th className="py-2 px-4 border-b border-gray-500">Phone Number</th>
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.email} className="hover:bg-gray-300">
                    ...
                    <td className="py-2 px-4 border-b border-gray-500">{student.phonenumber}</td>
          ...
          <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
            <div className="bg-white p-6 rounded-lg max-w-lg w-full">
              ...
              <form onSubmit={handleEditSubmit}>
                ...
                <div className="mb-4">
                  <label htmlFor="starting" className="block text-sm font-medium text-gray-700">Start Date:</label>
                  <input
                    type="date"
                    id="starting"
                    name="starting"
