...
const Bca = () => {
  ...
  const handleSubscription = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://courseapi-3kus.onrender.com/api/student/${p}`, { "bca": true ,"course":"BCA","starting":time,"end":result});
      setLock(true);
      setErrorMessage('');
      setPay(!pay)
    } catch (error) {
          ...
          <div>
            ...
            {lock && link ? (
                ...
                {lock && (<div className='flex flex-col gap-4'>
                  ...
                  <button className="bg-primary text-white bg-orange-500 cursor-pointer hover:scale-105 duration-300 py-2 px-8 rounded-full" onClick={() => navigate('/bcaeaxm@24')}>
                    Exam
                  </button>
                  </div>
                )}
