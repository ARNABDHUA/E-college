  ...
  useEffect(() => {
    axios.get('https://courseapi-3kus.onrender.com/api/products?sub=bcaLIVE')
      .then(res => {
        ...
        const filteredRecords = res.data.mydata.filter(record => record.date === fixt);
                      ...
                      <tbody>
                        {records.map((d, i) => (
                          <tr key={i}>
                            ...
                            <td className="border-2 border-gray-400 p-2 rounded-md w-80 sm:w-auto bg-red-500 cursor-pointer" onClick={() => attend(d.teacher, d.subtitle, d.link)}>live {d.time}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
